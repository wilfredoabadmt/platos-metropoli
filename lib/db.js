...
// Complementar votos hasta alcanzar exactamente el tope de 150 con distribución heterogénea
    ...
    try {
        ...
        for (const [dishId, target] of Object.entries(targetCounts)) {
            ...
            for (let i = 0; i < needed; i++) {
                ...
                const uuid = `voter_bolivia_${Date.now().toString(36)}_${seedCounter}_${Math.random().toString(36).slice(2, 6)}`;
        ...
        // Si faltasen votos adicionales para completar exactamente 150
        let remainingTo150 = 150 - Number(db.prepare('SELECT COUNT(*) as count FROM votes').get().count);
        const dishIds = ['fiambre', 'apthapi', 'wallake', 'aji-fideo', 'pesque', 'sopa-fideo'];
        let idx = 0;
        while (remainingTo150 > 0) {
            const dishId = dishIds[idx % dishIds.length];
            const district = allDistricts[(seedCounter * 11) % allDistricts.length];
            const city = district.includes('La Paz') ? 'La Paz' : (district.includes('Otra') ? 'Cochabamba' : 'El Alto');
            const ip = `190.181.${30 + (seedCounter % 60)}.${10 + (seedCounter % 200)}`;
            const uuid = `voter_seed_final_${Date.now()}_${seedCounter}`;
            insertVoteStmt.run(dishId, district, city, 'Bolivia', 'Móvil', ip, uuid, 'Mozilla/5.0 (Android; Mobile)', new Date().toISOString());
            remainingTo150--;
            seedCounter++;
            idx++;
        }
